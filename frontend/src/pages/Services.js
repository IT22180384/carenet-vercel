...
const Services = () => {
  ...
  const handleServiceUpdate = async (updatedService) => {
    try {
      const response = await fetch(
          `https://carenet-vercel-git-main-sandanimas-projects.vercel.app/serviceRoute/${selectedService._id}`,
          {
            method: "PUT",
            headers: {
  ...
  const handleDelete = async (serviceId) => {
    if (window.confirm("Are you sure you want to delete this service?")) {
      try {
        const response = await fetch(
            `https://carenet-vercel-git-main-sandanimas-projects.vercel.app/serviceRoute/${serviceId}`,
            {
              method: "DELETE",
            }
        );
